// Loads part details from the API and lets you + / - inventory, with recent log view.
(function () {
  ...
  async function loadLogs() {
    try {
      ...
      box.innerHTML = `<div class="ws-label" style="margin-bottom:6px;">Recent Inventory Activity</div>` +
        (r.logs || []).map(l =>
          `<div class="ws-row"><span>${l.ts}</span><span>${l.source}</span><span>${l.delta > 0 ? "+" : ""}${l.delta}</span></div>`
        ).join("");
    } catch { /* ignore */ }
  }
